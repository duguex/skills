...
Inside a round every question arrives through the ask tool (`AskUserQuestion` in ZCode, `ask` in Claude Code): each decision is a numbered question with 2–5 concrete options, the tradeoffs spelled out in the option descriptions, and the agent's recommendation first, marked `(Recommended)`. That is what makes a round answerable by number — "1 yes, 2 the second option, 3 no, here's why" — instead of by quoting questions back. The format has one known rough edge: the recommendation sometimes argues *against* the question as it was worded, so agreeing with the recommendation still lands on the recommended option rather than a plain "yes". When that happens, pick the recommended option and say so.
...
- A round arrives as one ask-tool call — each decision a numbered question with concrete options and a marked `(Recommended)` — and you can answer the whole round by number.
